...
const products = [
  ...
  },
  {
    ...
    "description": "ATV model from the supplied English ATV pricelist. Wholesale quotation, availability and customization available from Slow Rider.",
    "specs": {
  ...
  list.forEach(product => {
    ...
    card.innerHTML = `
      ...
      <div class="product-content">
        ...
        <button type="button" class="product-details-button">View Details</button>
      </div>`;

    setProductImage(card.querySelector(".catalog-product-image"), product);
...
function openProduct(id) {
  ...
  modal.innerHTML = `
    <div class="modal-box" role="dialog" aria-modal="true" aria-label="${product.name}">
      <button type="button" class="close-modal" onclick="closeProduct()" aria-label="Close product details">×</button>
      ...
      <div class="modal-product-info">
        ...
        <div class="modal-actions">
          <button type="button" class="whatsapp-btn" onclick="askQuote('${product.name.replace(/'/g, "\\'")}')">
            Request Quote on WhatsApp
          </button>
          <button type="button" class="close-product-button" onclick="closeProduct()">Close</button>
        </div>
      </div>
    </div>`;

  setProductImage(document.getElementById("modalProductImage"), product);
  ...
  if (quoteForm) {
    quoteForm.addEventListener("submit", event => {
      ...
      const lines = [
        ...
        `WhatsApp / Phone: ${value("phone") || "Not specified"}`,
        `Estimated Order Quantity: ${value("quantity") || "Not specified"}`,
        `Product Interested In: ${value("interest")}`,
        "",
        "Requirements:",
      ...
      window.open(
        "https://wa.me/8618620284214?text=" + encodeURIComponent(lines.join("\n")),
        "_blank",
        "noopener,noreferrer"
      );
    });
