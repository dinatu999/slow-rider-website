/* SLOW RIDER - PRODUCT CATALOG + WHATSAPP */
const products = [
  {
    "id": "patrol",
...
function getImageCandidates(product) {
  ...
  add(path.replace("__配置xlsx_", "_配置_"));
  add(path.replace("_配置_", "__配置xlsx_"));
...
function isElectricATV(product) {
  if (!product || product.category !== "ATV") return false;
  return Boolean(product.type === "Electric ATV" || product.specs?.Motor || product.name.toLowerCase().includes("electric"));
}

function productMatches(product, category) {
  if (category === "all") return true;
  if (category === "ATV") return product.category === "ATV";
  if (category === "Gas ATV") return product.category === "ATV" && !isElectricATV(product);
  if (category === "Electric ATV") return isElectricATV(product);
  if (category === "Electric Mobility") {
    return ["Electric Scooter", "Electric Motorcycle", "E-Bike", "Utility Vehicle", "Cargo Vehicle", "Passenger Vehicle", "Three Wheel Vehicle"].includes(product.category);
  }
  if (category === "Commercial") {
    return ["Utility Vehicle", "Cargo Vehicle", "Passenger Vehicle", "Three Wheel Vehicle"].includes(product.category);
  }
...
function renderProducts(category = "all") {
  ...
  container.innerHTML = "";
  const list = products.filter(product => productMatches(product, category));

  if (!list.length) {
    container.innerHTML = `<div class="empty-products"><strong>No products in this category yet.</strong><span>Please contact Slow Rider for available models.</span></div>`;
    return;
  }
  ...
  list.forEach(product => {
    ...
    const label = product.category === "ATV" ? (isElectricATV(product) ? "Electric ATV" : "Gas ATV") : product.category;

    card.innerHTML = `
      <div class="product-image-wrapper">
        <span class="product-badge">${label}</span>
        <img class="catalog-product-image" alt="${product.name}" loading="lazy">
        <button class="image-view-btn" type="button" onclick="openProduct('${product.id}')" aria-label="View ${product.name}">View</button>
      </div>
      <div class="product-content">
        ...
        <div class="card-actions">
          <button class="details-btn" type="button" onclick="openProduct('${product.id}')">View Details</button>
          <button class="mini-quote" type="button" onclick="askQuote('${product.name.replace(/'/g, "\\'")}')">WhatsApp</button>
        </div>
      </div>
    `;

    loadProductImage(card.querySelector(".catalog-product-image"), product);
...
function askQuote(productName) {
  ...
  const url = "https://wa.me/8618620284214?text=" + encodeURIComponent(text);
...
function openProduct(id) {
  ...
  const specs = Object.entries(product.specs || {})
    .filter(([, value]) => value !== "")
    .map(([label, value]) => `<div class="spec-row"><strong>${label}</strong><span>${String(value).replace(/\n/g, "<br>")}</span></div>`)
    .join("");

  modal.innerHTML = `
    <div class="modal-box" role="dialog" aria-modal="true" aria-label="${product.name}">
      <button type="button" class="close-modal" onclick="closeProduct()" aria-label="Close">×</button>
      <div class="modal-layout">
        <div class="modal-image-wrapper">
          <span class="modal-label">${label}</span>
          <img id="modalProductImage" alt="${product.name}">
        </div>
        <div class="modal-info">
          ...
          <button type="button" class="whatsapp-btn" onclick="askQuote('${product.name.replace(/'/g, "\\'")}')">Request Quote on WhatsApp <span>→</span></button>
        </div>
      </div>
    </div>
  `;

  loadProductImage(document.getElementById("modalProductImage"), product);
...
document.addEventListener("click", event => {
  const modal = document.getElementById("productModal");
  if (modal && event.target === modal) closeProduct();
});
