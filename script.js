...
/* SLOW RIDER — COMPLETE BLUE B2B REDESIGN
   All existing product records are preserved below. */


const WHATSAPP_NUMBER = "8618620284214";
const categoryDefinitions = [
  ["all","All Products"], ["ATV","ATVs"], ["Gas ATV","Gas ATVs"], ["Electric ATV","Electric ATVs"],
  ["Kids ATV","Kids ATVs"], ["Utility ATV","Utility ATVs"], ["Sport ATV","Sport / 4x4 ATVs"],
  ["Electric Mobility","Electric Mobility"], ["Electric Scooter","Electric Scooters"], ["Electric Motorcycle","Electric Motorcycles"], ["Commercial","Commercial Vehicles"]
];
function esc(v){return String(v??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#039;"}[c]))}
function normalize(v){return String(v??"").toLowerCase().replace(/[–—‑]/g,"-")}
function isElectricATV(p){return p?.category==="ATV" && (Boolean(p.specs?.Motor)||/electric/i.test(p.name||""))}
function is125ccATV(p){if(p?.category!=="ATV"||isElectricATV(p))return false;return /125\s*cc/i.test(`${p.name} ${Object.values(p.specs||{}).join(" ")}`)}
function isSportATV(p){if(p?.category!=="ATV"||isElectricATV(p)||is125ccATV(p))return false;const t=normalize(`${p.name} ${Object.values(p.specs||{}).join(" ")}`);return /4wd|4x4|400\s*cc|350\s*cc|300\s*cc|250\s*cc/.test(t)}
function matchesCategory(p,f){if(!f||f==="all")return true;if(f==="ATV")return p.category==="ATV";if(f==="Electric ATV")return isElectricATV(p);if(f==="Gas ATV")return p.category==="ATV"&&!isElectricATV(p);if(f==="Kids ATV")return is125ccATV(p);if(f==="Sport ATV")return isSportATV(p);if(f==="Utility ATV")return p.category==="ATV"&&!isElectricATV(p)&&!is125ccATV(p)&&!isSportATV(p);if(f==="Electric Mobility")return p.category!=="ATV";if(f==="Commercial")return ["Utility Vehicle","Cargo Vehicle","Passenger Vehicle","Three Wheel Vehicle"].includes(p.category);return p.category===f}
function productLabel(p){if(p.category==="ATV"){if(isElectricATV(p))return "Electric ATV";if(is125ccATV(p))return "125cc ATV";if(isSportATV(p))return "Sport / 4x4 ATV";return "ATV"}return p.category}
function countCategory(f){return products.filter(p=>matchesCategory(p,f)).length}
function getImageCandidates(p){const path=String(p?.image||"").trim();const out=[];const add=v=>{if(v&&!out.includes(v))out.push(v)};if(!path)return out;add(path);add("./"+path);add(path.replace(/\s+/g,"%20"));add(path.replace("_配置_","__配置_"));add(path.replace("__配置_","_配置_"));add(path.replace("_配置xlsx_","_配置_"));add(path.replace("_配置_","_配置xlsx_"));if(/\.png$/i.test(path)){add(path.replace(/\.png$/i,".jpg"));add(path.replace(/\.png$/i,".jpeg"))}if(/\.jpe?g$/i.test(path)){add(path.replace(/\.jpe?g$/i,".png"))}return [...new Set(out)]}
function loadProductImage(img,p){if(!img||!p)return;const c=getImageCandidates(p);let i=0;const fallback="slow_rider_product_images/placeholder.png";img.classList.remove("image-failed");const next=()=>{if(i>=c.length){img.onerror=null;if(img.src!==new URL(fallback,document.baseURI).href)img.src=fallback;img.classList.add("image-failed");return}const raw=c[i++];let src=raw;try{src=new URL(raw,document.baseURI).href}catch(e){}img.onerror=next;img.src=src};if(img.getAttribute("src")){img.onerror=next;return}next()}
function findSpec(p,patterns){for(const [k,v] of Object.entries(p.specs||{})){if(v&&patterns.some(rx=>rx.test(k)))return [k,v]}return null}
function keySpecs(p){const picks=[];const groups=[[/(motor|engine|power)/i,"Power"],[/battery/i,"Battery"],[/(max\s*speed|speed)/i,"Max Speed"],[/range/i,"Range"],[/(tire|tyre)/i,"Tires"],[/brake/i,"Brakes"],[/fuel\s*tank/i,"Fuel Tank"],[/suspension|shock/i,"Suspension"]];for(const [rx,label] of groups){const hit=findSpec(p,[rx]);if(hit&&!picks.some(x=>x[0]===hit[0]))picks.push([hit[0],hit[1],label]);if(picks.length===4)break}return picks}
function cardHtml(p){const ks=keySpecs(p).slice(0,2);return `<article class="product-card"><div class="product-card-media"><span class="product-badge">${esc(productLabel(p))}</span><img class="js-product-image" data-image-id="${esc(p.id)}" alt="${esc(p.name)}" loading="lazy"></div><div class="product-card-body"><h3>${esc(p.name)}</h3><p>${esc(p.description||"Vehicle available for wholesale inquiry.")}</p><div class="quick-spec-row">${ks.map(([k,v,label])=>`<div class="quick-spec"><strong>${esc(v)}</strong><span>${esc(label||k)}</span></div>`).join("")||'<div class="quick-spec"><strong>Request details</strong><span>Specifications</span></div>'}</div><div class="card-actions"><a href="product.html?id=${encodeURIComponent(p.id)}">View Details</a><a class="primary-card" href="quote.html?product=${encodeURIComponent(p.id)}">Request Quote</a></div></div></article>`}
function hydrateImages(root=document){root.querySelectorAll(".js-product-image").forEach(img=>{const p=products.find(x=>x.id===img.dataset.imageId);if(p)loadProductImage(img,p)})}
...
function headerHtml(){const page=document.body.dataset.page||"";const active=n=>page===n?"active":"";return `<header class="site-header"><div class="container header-inner"><a class="brand" href="index.html"><span class="brand-mark">S</span><span class="brand-copy"><strong>SLOW RIDER</strong><small>ELECTRIC MOBILITY · GLOBAL B2B</small></span></a><form class="header-search" data-site-search><span>⌕</span><input type="search" name="q" placeholder="Search products…" aria-label="Search products"></form><button class="menu-toggle" id="menuToggle" type="button" aria-label="Open navigation" aria-expanded="false">☰</button><nav class="main-nav" id="mainNav"><a class="${active("home")}" href="index.html">HOME</a><a class="${active("products")}" href="products.html">PRODUCTS</a><a class="${active("about")}" href="about.html">ABOUT US</a><a class="${active("contact")}" href="contact.html">CONTACT</a><a class="nav-quote ${active("quote")}" href="quote.html">REQUEST A QUOTE</a><button class="language-button" type="button" aria-label="Language: English">EN</button></nav></div></header>`}
function footerHtml(){return `<footer class="site-footer"><div class="container footer-main"><div class="footer-brand"><a class="brand" href="index.html"><span class="brand-mark">S</span><span class="brand-copy"><strong>SLOW RIDER</strong><small>ELECTRIC MOBILITY · GLOBAL B2B</small></span></a><p>Electric mobility built for global markets. Product sourcing and inquiry support for dealers, distributors and business buyers.</p></div><div><h3 class="footer-title">Navigation</h3><div class="footer-links"><a href="index.html">Home</a><a href="products.html">Products</a><a href="about.html">About Us</a><a href="contact.html">Contact</a><a href="quote.html">Request a Quote</a></div></div><div><h3 class="footer-title">Product Categories</h3><div class="footer-links"><a href="products.html?category=Electric%20Scooter">Electric Scooters</a><a href="products.html?category=Electric%20Motorcycle">Electric Motorcycles</a><a href="products.html?category=Commercial">Commercial Vehicles</a><a href="products.html?category=ATV">ATVs</a></div></div><div class="footer-contact"><h3 class="footer-title">Contact</h3><p>Phone / WhatsApp</p><p style="color:#fff;font-weight:700;margin-top:5px">+86 186 2028 4214</p><div class="footer-actions"><a class="btn btn-primary" href="quote.html">Request a Quote</a><a class="btn btn-whatsapp" href="https://wa.me/${WHATSAPP_NUMBER}" target="_blank" rel="noopener">WhatsApp</a></div></div></div><div class="container footer-bottom"><span>© 2026 Slow Rider. All Rights Reserved.</span><span>China · Global Wholesale Supply</span></div></footer>`}
function initShell(){const h=document.getElementById("siteHeader"),f=document.getElementById("siteFooter");if(h)h.innerHTML=headerHtml();if(f)f.innerHTML=footerHtml();const menu=document.getElementById("menuToggle"),nav=document.getElementById("mainNav");if(menu&&nav){menu.addEventListener("click",()=>{const open=nav.classList.toggle("open");document.body.classList.toggle("menu-open",open);menu.setAttribute("aria-expanded",String(open));menu.textContent=open?"×":"☰"});nav.querySelectorAll("a").forEach(a=>a.addEventListener("click",()=>{nav.classList.remove("open");document.body.classList.remove("menu-open")}))}document.querySelectorAll("[data-site-search]").forEach(form=>form.addEventListener("submit",e=>{e.preventDefault();const q=new FormData(form).get("q")?.trim();if(q)location.href=`products.html?q=${encodeURIComponent(q)}`}))}

function initHome(){if(document.body.dataset.page!=="home")return;document.getElementById("homeProductCount").textContent=products.length;document.getElementById("homeAtvCount").textContent=countCategory("ATV");document.getElementById("homeElectricCount").textContent=countCategory("Electric Mobility");const cats=[['Electric Scooter','⚡','Electric Scooters','Urban and patrol-oriented electric scooter models.'],['Electric Motorcycle','◉','Electric Motorcycles','Electric motorcycle models with multiple power configurations.'],['Commercial','▣','Commercial Vehicles','Utility, cargo, passenger and three-wheel electric vehicles.'],['ATV','◇','ATVs','Electric and gasoline ATV models across multiple configurations.']];const catRoot=document.getElementById("homeCategories");catRoot.innerHTML=cats.map(([f,ic,title,desc])=>`<article><span class="category-icon">${ic}</span><h3>${title}</h3><p>${desc}</p><a class="category-meta" href="products.html?category=${encodeURIComponent(f)}"><span>${countCategory(f)} models</span><span>Explore →</span></a></article>`).join("");const picks=["big-cg125","maxi","sunrise","cg125"].map(id=>products.find(p=>p.id===id)).filter(Boolean);const root=document.getElementById("featuredProducts");root.innerHTML=picks.map(cardHtml).join("");hydrateImages(root)}

let currentFilter="all";
function searchMatches(p,q){if(!q)return true;const hay=normalize([p.name,p.category,p.description,...Object.entries(p.specs||{}).flat()].join(" "));return normalize(q).split(/\s+/).every(t=>hay.includes(t))}
function renderCatalog(){const root=document.getElementById("catalogView");if(!root)return;const q=document.getElementById("productSearch")?.value.trim()||"";const items=products.filter(p=>matchesCategory(p,currentFilter)&&searchMatches(p,q));root.innerHTML=items.map(cardHtml).join("");hydrateImages(root);document.getElementById("catalogResultCount").textContent=`${items.length} product${items.length===1?"":"s"}`;document.getElementById("catalogStatusText").textContent=q?`Search: “${q}”`:currentFilter==="all"?"Showing all products":`Category: ${categoryDefinitions.find(x=>x[0]===currentFilter)?.[1]||currentFilter}`;document.getElementById("catalogEmpty").hidden=items.length>0}
function initCatalog(){if(document.body.dataset.page!=="products"||!document.getElementById("catalogView"))return;document.getElementById("catalogTotal").textContent=products.length;const params=new URLSearchParams(location.search);currentFilter=params.get("category")||"all";if(!categoryDefinitions.some(x=>x[0]===currentFilter))currentFilter="all";const filters=document.getElementById("categoryFilters");filters.innerHTML=categoryDefinitions.map(([f,label])=>`<button type="button" class="filter-chip ${f===currentFilter?"active":""}" data-filter="${esc(f)}">${esc(label)} <b>${countCategory(f)}</b></button>`).join("");filters.addEventListener("click",e=>{const b=e.target.closest("[data-filter]");if(!b)return;currentFilter=b.dataset.filter;filters.querySelectorAll(".filter-chip").forEach(x=>x.classList.toggle("active",x===b));renderCatalog()});const search=document.getElementById("productSearch");search.value=params.get("q")||"";search.addEventListener("input",renderCatalog);document.getElementById("clearProductSearch").addEventListener("click",()=>{search.value="";search.focus();renderCatalog()});renderCatalog()}

function colorValueToCss(name){const n=normalize(name).trim();const map={black:"#111",blue:"#1674dc",red:"#dc3246",white:"#fff",gray:"#9aa5ad",grey:"#9aa5ad",green:"#3b8a58",orange:"#f18b2c",yellow:"#e8c71d",brown:"#80543a",purple:"#7652a7","military green":"#556b45"};return map[n]||"#dce7ee"}
function getColors(p){const hit=findSpec(p,[/^colors?$/i]);if(!hit)return[];return String(hit[1]).split(/[\/|,]/).map(x=>x.trim()).filter(Boolean)}
function featureItemsFromSpecs(p){const entries=Object.entries(p.specs||{}).filter(([,v])=>v);const preferred=[/lighting/i,/display|instrument|dashboard/i,/frame|appearance/i,/suspension|shock/i,/brake/i,/tire|tyre/i,/oem|odm/i,/additional|optional/i];const out=[];for(const rx of preferred){const hit=entries.find(([k])=>rx.test(k));if(hit&&!out.some(x=>x[0]===hit[0]))out.push(hit);if(out.length===7)break}if(out.length<5){for(const e of entries){if(!out.some(x=>x[0]===e[0]))out.push(e);if(out.length===7)break}}return out}
function detailIcon(label){const t=normalize(label);if(/motor|engine|power/.test(t))return "⚡";if(/battery/.test(t))return "▣";if(/speed/.test(t))return "◉";if(/range/.test(t))return "↔";if(/tire|tyre/.test(t))return "◎";if(/brake/.test(t))return "◇";if(/fuel/.test(t))return "◫";return "⌁"}
function galleryFor(p){const raw=Array.isArray(p.gallery)?p.gallery.filter(Boolean):[];const imgs=[p.image,...raw].filter(Boolean);return [...new Set(imgs)]}
function initProductPage(){const root=document.getElementById("productPage");if(!root)return;const id=new URLSearchParams(location.search).get("id");const p=products.find(x=>x.id===id)||products[0];document.title=`${p.name} | Slow Rider`;const ks=keySpecs(p);const colors=getColors(p);const gallery=galleryFor(p);const slots=Array.from({length:6},(_,i)=>gallery[i]?`<button class="product-thumb ${i===0?"active":""}" data-gallery-src="${esc(gallery[i])}" type="button"><img src="${esc(gallery[i])}" alt="${esc(p.name)} photo ${i+1}" loading="lazy"></button>`:`<div class="product-thumb placeholder"><strong>PHOTO ${i+1}</strong><small>COMING SOON</small></div>`).join("");const specRows=Object.entries(p.specs||{}).filter(([,v])=>v).map(([k,v])=>`<tr><td>${esc(k)}</td><td>${esc(v).replace(/\n/g,"<br>")}</td></tr>`).join("");const features=featureItemsFromSpecs(p).map(([k,v])=>`<li><strong>${esc(k)}:</strong> ${esc(v)}</li>`).join("");const strip=ks.map(([k,v,label])=>`<article><span>${detailIcon(k)}</span><strong>${esc(label||k)}</strong><small>${esc(v)}</small></article>`).join("");const related=products.filter(x=>x.id!==p.id&&(x.category===p.category||(p.category==="ATV"&&x.category==="ATV"))).slice(0,4);root.innerHTML=`<section class="product-detail-shell"><div class="container"><div class="product-breadcrumb"><a href="index.html">Home</a><span>›</span><a href="products.html">Products</a><span>›</span><a href="products.html?category=${encodeURIComponent(p.category)}">${esc(productLabel(p))}</a><span>›</span><strong>${esc(p.name)}</strong></div><div class="product-top"><div><div class="product-gallery-main"><span class="detail-badge">${esc(productLabel(p))}</span><img id="detailMainImage" src="${esc(gallery[0]||"slow_rider_product_images/placeholder.png")}" alt="${esc(p.name)}" loading="eager"><span class="image-count">${Math.max(1,gallery.length)} / 6</span></div><div class="product-thumbs" id="productThumbs">${slots}</div></div><div class="product-info-panel"><span class="detail-category-pill">${esc(productLabel(p))}</span><h1>${esc(p.name)}</h1><p class="product-tagline">Built for practical mobility and global B2B markets.</p><p class="product-description">${esc(p.description||"")}</p><div class="detail-spec-cards">${ks.map(([k,v,label])=>`<div class="detail-spec-card"><span class="spec-icon">${detailIcon(k)}</span><strong title="${esc(v)}">${esc(v)}</strong><span>${esc(label||k)}</span></div>`).join("")}</div><div class="detail-options"><div class="option-box"><label>Color Options</label>${colors.length?`<div class="color-row">${colors.map(c=>`<span class="color-dot" title="${esc(c)}" style="background:${colorValueToCss(c)}"></span>`).join("")}<span class="color-text">${esc(colors.join(" / "))}</span></div>`:`<span class="color-text">Confirm available colors with sales.</span>`}</div><div class="option-box"><label>OEM / ODM</label><div class="oem-line"><span class="spec-icon">▣</span><span>${esc((findSpec(p,[/oem|odm/i])||[null,"Discuss available options"])[1])}</span></div></div></div><div class="detail-actions"><a class="btn btn-primary" href="quote.html?product=${encodeURIComponent(p.id)}">Request a Quote</a><a class="btn btn-whatsapp" href="${whatsappUrl(`Hello Slow Rider, I am interested in ${p.name}. Please send me more information and a quotation.`)}" target="_blank" rel="noopener">Chat on WhatsApp</a></div><div class="support-row"><span><b>◎</b> Global shipping discussion</span><span><b>◇</b> Product support</span><span><b>⌁</b> Spare parts / after-sales discussion</span></div></div></div><div class="feature-strip">${strip}</div><div class="product-lower"><section class="lower-card"><h2>Specifications</h2><table class="spec-table"><tbody>${specRows}</tbody></table></section><section class="lower-card"><h2>Product Features</h2><ul class="feature-list">${features}</ul></section><aside class="lower-card promo-card"><div><span class="eyebrow">EXPLORE</span><h3>MORE<br>POSSIBILITIES</h3><p>Wholesale · OEM / ODM · Product configuration discussions</p></div><img class="js-promo-image" alt="${esc(p.name)}"></aside></div></div></section><section class="related-section"><div class="container"><div class="related-head"><div><span class="eyebrow">YOU MAY ALSO LIKE</span><h2>Related Products</h2></div><a class="text-link" href="products.html">View all products →</a></div><div class="product-grid" id="relatedProducts">${related.map(cardHtml).join("")}</div></div></section>`;loadProductImage(document.getElementById("detailMainImage"),p);const promo=document.querySelector(".js-promo-image");loadProductImage(promo,p);const thumbButtons=root.querySelectorAll("[data-gallery-src]");thumbButtons.forEach((btn,i)=>{const img=btn.querySelector("img");const fake={...p,image:gallery[i]};loadProductImage(img,fake);btn.addEventListener("click",()=>{thumbButtons.forEach(x=>x.classList.remove("active"));btn.classList.add("active");loadProductImage(document.getElementById("detailMainImage"),fake)})});hydrateImages(document.getElementById("relatedProducts"))}

function initAbout(){if(document.body.dataset.page!=="about")return;const root=document.getElementById("aboutCategoryGrid");if(!root)return;const cats=[['⚡','Electric Scooters','Electric Scooter'],['◉','Electric Motorcycles','Electric Motorcycle'],['▣','Commercial Vehicles','Commercial'],['◇','ATVs','ATV']];root.innerHTML=cats.map(([ic,title,f])=>`<article><span>${ic}</span><h3>${title}</h3><p>${countCategory(f)} current product records in this category.</p></article>`).join("")}
function initQuoteForm(){const select=document.getElementById("quoteProduct");if(select){products.forEach(p=>{const o=document.createElement("option");o.value=p.name;o.textContent=`${p.name} — ${productLabel(p)}`;select.appendChild(o)});const id=new URLSearchParams(location.search).get("product");const p=products.find(x=>x.id===id);if(p)select.value=p.name}const form=document.getElementById("quoteForm");if(form)form.addEventListener("submit",e=>{e.preventDefault();const d=Object.fromEntries(new FormData(form));openWhatsApp(`Hello Slow Rider,\n\nI would like to request a wholesale quotation.\n\nName: ${d.name||""}\nCompany: ${d.company||""}\nCountry: ${d.country||""}\nPhone / WhatsApp: ${d.phone||""}\nEstimated Quantity: ${d.quantity||""}\nInterested Product: ${d.product||""}\n\nRequirements:\n${d.requirements||""}`)})}
function initContactForm(){const form=document.getElementById("contactForm");if(!form)return;form.addEventListener("submit",e=>{e.preventDefault();const d=Object.fromEntries(new FormData(form));openWhatsApp(`Hello Slow Rider,\n\nI would like to make an inquiry.\n\nName: ${d.name||""}\nCompany: ${d.company||""}\nCountry: ${d.country||""}\nPhone / WhatsApp: ${d.phone||""}\nInterested Product: ${d.product||""}\n\nMessage:\n${d.message||""}`)})}

document.addEventListener("DOMContentLoaded",()=>{initShell();initHome();initCatalog();initProductPage();initAbout();initQuoteForm();initContactForm()});
