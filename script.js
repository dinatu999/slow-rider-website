...
function loadProductImage(img,p){if(!img||!p)return;const c=getImageCandidates(p);let i=0;const fallback="slow_rider_product_images/placeholder.png";img.classList.remove("image-failed");const next=()=>{if(i>=c.length){img.onerror=null;img.classList.add("image-failed");img.src=fallback;return}const src=c[i++];img.onerror=next;img.src=src.startsWith("./")||src.includes("%20")?src:encodeURI(src)};next()}
