...
function openProduct(id){const p=products.find(x=>x.id===id),m=document.getElementById('productModal');if(!p||!m)return;const label=labelFor(p);const specs=Object.entries(p.specs||{}).filter(([,v])=>v).map(([k,v])=>`<div class="spec-row"><strong>${esc(k)}</strong><span>${esc(v).replace(/\n/g,'<br>')}</span></div>`).join('');m.innerHTML=`<div class="modal-box" role="dialog" aria-modal="true"><button class="close-modal" data-close-modal aria-label="Close">×</button><div class="modal-layout"><div class="modal-image-wrapper"><span class="modal-label">${esc(label)}</span><img id="modalProductImage" alt="${esc(p.name)}"></div><div class="modal-info"><span class="product-category">${esc(label)}</span><h2>${esc(p.name)}</h2><p class="modal-description">${esc(p.description||'Wholesale vehicle available for international dealers and distributors.')}</p><h3>Specifications</h3><div class="spec-list">${specs}</div><button class="whatsapp-btn" data-wa-product="${esc(p.name)}">◉ &nbsp; Send Inquiry via WhatsApp</button><button class="modal-close-bottom" data-close-modal>Close</button></div></div></div>`;loadProductImage(document.getElementById('modalProductImage'),p);m.classList.add('is-open');m.setAttribute('aria-hidden','false');document.body.classList.add('modal-open')}
...
document.addEventListener('click',e=>{const detail=e.target.closest('[data-product-id]');if(detail){openProduct(detail.dataset.productId);return}const wa=e.target.closest('[data-wa-product]');if(wa){askQuote(wa.dataset.waProduct);return}const action=e.target.closest('[data-filter-action]');if(action){filterProducts(action.dataset.filterAction);return}const filter=e.target.closest('[data-filter]');if(filter){filterProducts(filter.dataset.filter);return}const close=e.target.closest('[data-close-modal]');if(close){closeProduct();return}const modal=document.getElementById('productModal');if(modal&&e.target===modal)closeProduct()});
