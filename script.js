...
function isElectricATV(p){return p?.category==='ATV'&&(p.type==='Electric ATV'||p.specs?.Motor||/electric/i.test(p.name||''))}
...
function filterProducts(filter){activateFilter(filter);document.getElementById('products')?.scrollIntoView({behavior:'smooth',block:'start'})}
...
document.addEventListener('DOMContentLoaded',()=>{updateCounts();renderCatalog('all');const s=document.getElementById('mobileFilter');if(s)s.addEventListener('change',()=>filterProducts(s.value));const menu=document.querySelector('.menu-toggle'),nav=document.getElementById('mainNav');if(menu&&nav)menu.addEventListener('click',()=>{const open=nav.classList.toggle('mobile-open');menu.setAttribute('aria-expanded',String(open));menu.textContent=open?'×':'☰'});document.querySelectorAll('.main-nav a[href^="#"]').forEach(a=>a.addEventListener('click',()=>nav?.classList.remove('mobile-open')))})
