...
<h3>MORE<br>POSSIBILITIES</h3>

<p class="promo-intro">
Wholesale · OEM / ODM · Product configuration discussions
</p>

<div class="promo-points">
    <span>✓ Custom configuration support</span>
    <span>✓ Dealer & wholesale solutions</span>
    <span>✓ Global market cooperation</span>
</div></div></aside></div></div></section><section class="related-section"><div class="container"><div class="related-head"><div><span class="eyebrow">YOU MAY ALSO LIKE</span><h2>Related Products</h2></div><a class="text-link" href="products.html">View all products →</a></div><div class="product-grid" id="relatedProducts">${related.map(cardHtml).join("")}</div></div></section>`;loadProductImage(document.getElementById("detailMainImage"),p);const thumbButtons=root.querySelectorAll("[data-gallery-src]");thumbButtons.forEach((btn,i)=>{const img=btn.querySelector("img");const fake={...p,image:gallery[i]};loadProductImage(img,fake);btn.addEventListener("click",()=>{thumbButtons.forEach(x=>x.classList.remove("active"));btn.classList.add("active");loadProductImage(document.getElementById("detailMainImage"),fake)})});hydrateImages(document.getElementById("relatedProducts"))}
