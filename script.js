/* SLOW RIDER - CLEAN CATALOG SCRIPT */
const products = [
  {
    "id": "patrol",
...
function getImageCandidates(p){const path=p?.image||'';if(!path)return[];const out=[];const add=v=>{if(v&&!out.includes(v))out.push(v)};add(path);add(path.replace('_配置_','__配置_'));add(path.replace('__配置_','_配置_'));add(path.replace('_配置xlsx_','_配置_'));add(path.replace('_配置_','_配置xlsx_'));if(/\.png$/i.test(path)){add(path.replace(/\.png$/i,'.jpg'));add(path.replace(/\.png$/i,'.jpeg'))}if(/\.jpg$/i.test(path)){add(path.replace(/\.jpg$/i,'.png'));add(path.replace(/\.jpg$/i,'.jpeg'))}return out}
function loadProductImage(img,p){const candidates=getImageCandidates(p);let i=0;const next=()=>{if(i>=candidates.length){img.onerror=null;img.classList.add('image-failed');return}img.src=encodeURI(candidates[i++])};img.onerror=next;next()}
function esc(s){return String(s??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#039;'}[c]))}
function isElectricATV(p){return p?.category==='ATV'&&(Boolean(p.specs?.Motor)||/electric/i.test(p.name||''))}
function is125ccATV(p){if(p?.category!=='ATV'||isElectricATV(p))return false;const text=(p.name+' '+' '+Object.entries(p.specs||{}).map(([k,v])=>k+' '+v).join(' '));return /125\s*cc/i.test(text)}
function isSportATV(p){if(p?.category!=='ATV'||isElectricATV(p)||is125ccATV(p))return false;const text=(p.name+' '+' '+Object.entries(p.specs||{}).map(([k,v])=>k+' '+v).join(' ')).toLowerCase();return /4wd|4x4|400\s*cc|300\s*cc|250\s*cc/.test(text)}
function matches(p,f){if(f==='all')return true;if(f==='ATV')return p.category==='ATV';if(f==='Kids ATV')return is125ccATV(p);if(f==='Electric ATV')return isElectricATV(p);if(f==='Gas ATV')return p.category==='ATV'&&!isElectricATV(p);if(f==='Sport ATV')return isSportATV(p);if(f==='Utility ATV')return p.category==='ATV'&&!isElectricATV(p)&&!is125ccATV(p)&&!isSportATV(p);if(f==='Electric Mobility')return p.category!=='ATV';if(f==='Commercial')return ['Utility Vehicle','Cargo Vehicle','Passenger Vehicle','Three Wheel Vehicle'].includes(p.category);return p.category===f}
...
function productCard(p){const label=labelFor(p);const specs=Object.entries(p.specs||{}).filter(([,v])=>v).slice(0,2).map(([k,v])=>esc(k)+': '+esc(v)).join(' • ');return `<article class="product-card"><div class="product-image-wrapper"><span class="product-badge">${esc(label)}</span><img class="catalog-product-image" alt="${esc(p.name)}" loading="lazy"></div><div class="product-content"><h3>${esc(p.name)}</h3><div class="product-meta">${esc(specs||p.description||'Wholesale vehicle')}</div><div class="product-bottom"><span class="product-price">Request quote</span><button class="details-btn" data-product-id="${esc(p.id)}">View Details</button><button class="mini-wa" data-wa-product="${esc(p.name)}">WhatsApp</button></div></div></article>`}
const META={ATV:['ATVs','🏍','orange'], 'Kids ATV':['Kids ATVs • 125cc','🏍','orange'],'Utility ATV':['Utility ATVs','🏍','orange'],'Sport ATV':['Sport / 4x4 ATVs','🏍','orange'],'Electric ATV':['Electric ATVs','⚡','green'],'Gas ATV':['Gas / Petrol ATVs','🏍','orange'],'Electric Mobility':['Electric Vehicles','⚡','green'],'Electric Scooter':['Electric Scooters','🛵','green'],'Electric Motorcycle':['Electric Motorcycles','🏍','green'],'Utility Vehicle':['Electric Utility Vehicles','◉','green'],'Cargo Vehicle':['Electric Cargo Vehicles','▣','green'],'Passenger Vehicle':['Passenger Vehicles','🚐','green'],'Three Wheel Vehicle':['Electric 3-Wheel Vehicles','♧','green'],Commercial:['Commercial Vehicles','◉','orange']};
function sectionBlock(title,icon,items,color,max=null,moreFilter=null){const shown=max?items.slice(0,max):items;const grid=shown.map(productCard).join('');const more=max&&items.length>max?`<div class="catalog-more"><button class="view-all-btn" data-filter-action="${esc(moreFilter)}">View All ${esc(title)} →</button></div>`:'';return `<section class="catalog-section"><div class="section-title-row ${color==='green'?'green':''}"><h2 class="section-title"><span class="section-icon">${icon}</span>${esc(title)}</h2><span class="count-pill">${items.length} Products</span></div><div class="product-grid">${grid||'<div class="empty-products">No products available in this category yet.</div>'}</div>${more}</section>`}
function renderCatalog(filter='all'){const root=document.getElementById('catalogView');if(!root)return;let blocks=[];if(filter==='all'){const atvs=products.filter(p=>p.category==='ATV');const ev=products.filter(p=>p.category!=='ATV');blocks=[sectionBlock('ATVs','🏍',atvs,'orange',6,'ATV'),sectionBlock('Electric Vehicles','⚡',ev,'green',6,'Electric Mobility')]}else{const items=products.filter(p=>matches(p,filter));const meta=META[filter]||[filter,'•','orange'];blocks=[sectionBlock(meta[0],meta[1],items,meta[2])]}root.innerHTML=blocks.join('');root.querySelectorAll('.catalog-product-image').forEach((img,i)=>{const cards=[...root.querySelectorAll('.product-card')];const id=cards[i]?.querySelector('[data-product-id]')?.dataset.productId;const p=products.find(x=>x.id===id);if(p)loadProductImage(img,p)})}
function activateFilter(filter){document.querySelectorAll('[data-filter]').forEach(b=>b.classList.toggle('active',b.dataset.filter===filter));const s=document.getElementById('mobileFilter');if(s)s.value=filter;renderCatalog(filter)}
function filterProducts(filter){activateFilter(filter);const target=document.getElementById('products');if(target)target.scrollIntoView({behavior:'smooth',block:'start'})}
...
function productHighlights(p){
  ...
  const find=(keys,fallback)=>{
    for(const key of keys){
      const hit=Object.entries(specs).find(([k,v])=>new RegExp(key,'i').test(k) && v);
      if(hit)return hit[1];
    }
    return fallback;
  };
  return [
    ['⚡','Motor',find(['motor','engine'],'Available to order')],
    ['🔋','Battery',find(['battery'],'Multiple options')],
    ['◉','Max Speed',find(['max speed','speed'],'Available on request')],
    ['◌','Brakes',find(['brakes','brake'],'See specifications')]
  ];
}

function relatedProducts(p){
  return products.filter(x=>x.id!==p.id && (x.category===p.category || (p.category==='ATV' && x.category==='ATV'))).slice(0,4);
}

function openProduct(id){
  const p=products.find(x=>x.id===id),m=ensureProductModal();
  if(!p)return;
  const label=labelFor(p);
  const specs=Object.entries(p.specs||{}).filter(([,v])=>v).map(([k,v])=>`<div class="detail-spec-row"><strong>${esc(k)}</strong><span>${esc(v).replace(/\n/g,'<br>')}</span></div>`).join('');
  const highlights=productHighlights(p).map(([icon,name,value])=>`<div class="detail-highlight"><span class="detail-highlight-icon">${icon}</span><div><strong>${esc(name)}</strong><span>${esc(value)}</span></div></div>`).join('');
  const thumbs=Array.from({length:6},(_,i)=>i===0
    ? `<button class="detail-thumb active" type="button" data-gallery-index="0"><img id="detailThumb0" alt="${esc(p.name)}"></button>`
    : `<button class="detail-thumb detail-thumb-empty" type="button" disabled><span>PHOTO ${i+1}</span><small>COMING SOON</small></button>`
  ).join('');
  const related=relatedProducts(p).map(r=>`<article class="detail-related-card"><div class="detail-related-image"><img class="detail-related-img" alt="${esc(r.name)}"></div><div><span>${esc(labelFor(r))}</span><h4>${esc(r.name)}</h4><button type="button" data-product-id="${esc(r.id)}">View Details →</button></div></article>`).join('');
  const faq=`<div class="detail-faq"><div><strong>Can I order samples?</strong><p>Yes. Sample and trial orders can be discussed with our sales team.</p></div><div><strong>Can you customize the vehicle?</strong><p>OEM / ODM and configuration options are available for selected models.</p></div><div><strong>Do you support international shipping?</strong><p>We supply dealers and distributors in international markets. Shipping terms are confirmed with each quotation.</p></div></div>`;

  m.innerHTML=`
    <div class="detail-page" role="dialog" aria-modal="true" aria-label="${esc(p.name)}">
      <button class="detail-close" type="button" data-close-modal aria-label="Close product details">×</button>
      <div class="detail-breadcrumb">Home <span>›</span> Products <span>›</span> ${esc(label)} <span>›</span> <strong>${esc(p.name)}</strong></div>
      <div class="detail-main">
        <section class="detail-gallery">
          <div class="detail-main-image-wrap">
            <span class="detail-badge">${esc(label)}</span>
            <img id="detailMainImage" alt="${esc(p.name)}">
            <span class="detail-image-count">1 / 6</span>
          </div>
          <div class="detail-thumbs">${thumbs}</div>
        </section>
        <section class="detail-info">
          <span class="detail-category">${esc(label)}</span>
          <h1>${esc(p.name)}</h1>
          <p class="detail-tagline">Reliable performance. Practical design. Built for global markets.</p>
          <p class="detail-description">${esc(p.description||'A practical vehicle platform for international dealers, distributors and fleet applications.')}</p>
          <div class="detail-quote-line"><strong>WHOLESALE / OEM</strong><span>Request current pricing and availability</span></div>
          <div class="detail-highlights">${highlights}</div>
          <div class="detail-actions">
            <button type="button" class="detail-quote-btn" data-wa-product="${esc(p.name)}">✉ &nbsp; Get a Quote</button>
            <button type="button" class="detail-wa-btn" data-wa-product="${esc(p.name)}">◉ &nbsp; Chat on WhatsApp</button>
          </div>
          <p class="detail-note">Custom colors, logo, packaging and configurations can be discussed for bulk orders.</p>
        </section>
      </div>
      <div class="detail-tabs" role="tablist">
        <button class="active" type="button" data-detail-tab="overview">Overview</button>
        <button type="button" data-detail-tab="specifications">Specifications</button>
        <button type="button" data-detail-tab="shipping">Shipping &amp; Payment</button>
        <button type="button" data-detail-tab="faq">FAQ</button>
      </div>
      <section class="detail-tab-panel active" data-detail-panel="overview">
        <div class="detail-overview-grid">
          <div><span class="detail-section-kicker">BUILT FOR BUSINESS</span><h2>Professional vehicles for real-world use.</h2><p>${esc(p.description||'Designed for practical transportation, dealer sales and fleet applications.')}</p><ul><li>Dealer and distributor friendly</li><li>OEM / ODM options available</li><li>Multiple battery and configuration choices</li><li>International wholesale support</li></ul></div>
          <div class="detail-overview-specs"><h3>Key Specifications</h3>${highlights}</div>
          <div class="detail-overview-photo"><div class="detail-photo-placeholder">PRODUCT<br>PHOTO</div><p>More product photos will be added here.</p></div>
        </div>
      </section>
      <section class="detail-tab-panel" data-detail-panel="specifications"><div class="detail-spec-list">${specs||'<p>No specifications have been added yet.</p>'}</div></section>
      <section class="detail-tab-panel" data-detail-panel="shipping"><div class="detail-info-columns"><div><h3>Shipping</h3><p>International shipping options are available. Final freight, lead time and delivery terms depend on destination and order quantity.</p></div><div><h3>Payment</h3><p>Payment terms are confirmed with the quotation. Contact us for current wholesale terms and order requirements.</p></div><div><h3>Bulk Orders</h3><p>For dealer and distributor orders, ask us about OEM branding, packaging and configuration options.</p></div></div></section>
      <section class="detail-tab-panel" data-detail-panel="faq">${faq}</section>
      <section class="detail-related"><div class="detail-related-head"><div><span class="detail-section-kicker">EXPLORE MORE</span><h2>You May Also Like</h2></div><button type="button" data-close-modal>View All Products →</button></div><div class="detail-related-grid">${related||'<p>More products will be added soon.</p>'}</div></section>
    </div>`;

  loadProductImage(document.getElementById('detailMainImage'),p);
  loadProductImage(document.getElementById('detailThumb0'),p);
  m.querySelectorAll('.detail-related-img').forEach((img,i)=>{const r=relatedProducts(p)[i];if(r)loadProductImage(img,r)});
  m.classList.add('is-open');m.setAttribute('aria-hidden','false');document.body.classList.add('modal-open');
}
...
document.addEventListener('click',e=>{const detail=e.target.closest('[data-product-id]');if(detail){openProduct(detail.dataset.productId);return}const wa=e.target.closest('[data-wa-product]');if(wa){askQuote(wa.dataset.waProduct);return}const action=e.target.closest('[data-filter-action]');if(action){filterProducts(action.dataset.filterAction);return}const filter=e.target.closest('[data-filter]');if(filter){filterProducts(filter.dataset.filter);return}const tab=e.target.closest('[data-detail-tab]');if(tab){const modal=e.target.closest('#productModal');if(modal){modal.querySelectorAll('[data-detail-tab]').forEach(b=>b.classList.toggle('active',b===tab));modal.querySelectorAll('[data-detail-panel]').forEach(p=>p.classList.toggle('active',p.dataset.detailPanel===tab.dataset.detailTab));}return}const close=e.target.closest('[data-close-modal]');if(close){closeProduct();return}const modal=document.getElementById('productModal');if(modal&&e.target===modal)closeProduct()});
document.addEventListener('keydown',e=>{if(e.key==='Escape')closeProduct()});
document.addEventListener('DOMContentLoaded',()=>{updateCounts();if(document.getElementById('catalogView')){const initial=new URLSearchParams(location.search).get('category')||'all';renderCatalog(initial)}const featured=document.getElementById('featuredProducts');if(featured){const picks=products.slice(0,6);featured.innerHTML=picks.map(productCard).join('');featured.querySelectorAll('.catalog-product-image').forEach((img,i)=>loadProductImage(img,picks[i]))}const s=document.getElementById('mobileFilter');if(s)s.addEventListener('change',()=>filterProducts(s.value));const menu=document.querySelector('.menu-toggle'),nav=document.getElementById('mainNav');if(menu&&nav)menu.addEventListener('click',()=>{const open=nav.classList.toggle('mobile-open');menu.setAttribute('aria-expanded',String(open));menu.textContent=open?'×':'☰'});document.querySelectorAll('.main-nav a').forEach(a=>a.addEventListener('click',()=>nav?.classList.remove('mobile-open')))})
