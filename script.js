...
function getImageCandidates(p){const path=p?.image||"";const out=[];const add=v=>{if(v&&!out.includes(v))out.push(v)};add(path);add(path.replace("_配置_","__配置_"));add(path.replace("__配置_","_配置_"));add(path.replace("_配置xlsx_","_配置_"));add(path.replace("_配置_","_配置xlsx_"));if(/\.png$/i.test(path)){add(path.replace(/\.png$/i,".jpg"));add(path.replace(/\.png$/i,".jpeg"))}if(/\.jpe?g$/i.test(path)){add(path.replace(/\.jpe?g$/i,".png"))}return out}
function loadProductImage(img,p){if(!img||!p)return;const c=getImageCandidates(p);let i=0;const fallback="slow_rider_product_images/placeholder.png";const next=()=>{if(i>=c.length){img.onerror=null;img.classList.add("image-failed");img.src=fallback;return}img.src=encodeURI(c[i++])};img.onerror=next;next()}
