...
function initProductPage(){
  ...
  document.title = `${p.name} | Slow Rider`;

  const ks = keySpecs(p);
  const colors = getColors(p);
  const gallery = galleryFor(p);

  const mainImage = gallery[0] || p.image || "slow_rider_product_images/placeholder.png";
    ...
    if (gallery[i]) {
      return `
        <button
          ...
          data-gallery-src="${esc(gallery[i])}"
          type="button">
          <img
            src="${esc(gallery[i])}"
            alt="${esc(p.name)} photo ${i + 1}"
            loading="eager"
            onerror="this.onerror=null;this.src='slow_rider_product_images/placeholder.png';">
        </button>
      `;
    }
  ...
  const features = featureItemsFromSpecs(p)
    .map(([k,v]) => `
      <li>
  ...
  const strip = ks
    .map(([k,v,label]) => `
      <article>
        <span>${detailIcon(k)}</span>
        <strong>${esc(label || k)}</strong>
        <small>${esc(v)}</small>
      </article>
    `)
    .join("");
    ...
    <section class="product-detail-shell">
      ...
      <div class="container">
        ...
        <div class="product-breadcrumb">
          ...
          <span>›</span>
          <a href="products.html?category=${encodeURIComponent(p.category)}">
            ${esc(productLabel(p))}
          </a>
          <span>›</span>
        ...
        <div class="product-top">
          ...
          <div class="product-info-panel">
            ...
            <span class="detail-category-pill">
              ${esc(productLabel(p))}
            </span>
            ...
            <p class="product-tagline">
              Built for practical mobility and global B2B markets.
            </p>
            ...
            <div class="detail-options">
              ...
              <div class="option-box">
                ...
                ${
                    ...
                    <div class="color-row">
                      ...
                      ${colors.map(c => `
                        <span
                          class="color-dot"
                          title="${esc(c)}"
                          style="background:${colorValueToCss(c)}">
                        </span>
                      `).join("")}
                  ...
                  `
                    <span class="color-text">
                      Confirm available colors with sales.
                    </span>
                  `
                }
              ...
              <div class="option-box">
                ...
                <div class="oem-line">
                  ...
                  <span>
                    ${
                      esc(
                        (findSpec(p,[/oem|odm/i]) ||
                        [null,"Discuss available options"])[1]
                      )
                    }
                  </span>
            ...
            <div class="detail-actions">
              ...
              <a
                ...
                href="quote.html?product=${encodeURIComponent(p.id)}">
                Request a Quote
              </a>
              ...
              <a
                ...
                href="${whatsappUrl(
                  `Hello Slow Rider, I am interested in ${p.name}. Please send me more information and a quotation.`
                )}"
                target="_blank"
                rel="noopener">
                Chat on WhatsApp
            ...
            <div class="support-row">
              ...
              <span>
                <b>◎</b> Global shipping discussion
              </span>
              ...
              <span>
                <b>⌁</b> Spare parts / after-sales discussion
              </span>
        ...
        <div class="product-lower">
          ...
          <aside class="lower-card promo-card">
  ...
  /*
   * MAIN IMAGE
   * Directly uses the product's image path.
   * No lazy-loading and no JavaScript image loader required.
   */
  ...
  if (detailImage) {
    detailImage.onerror = function(){
      this.onerror = null;
      this.src = "slow_rider_product_images/placeholder.png";
    };
   ...
   * PROMOTIONAL IMAGE
   */
  const promo = root.querySelector(".js-promo-image");

  if (promo) {
    promo.onerror = function(){
      this.onerror = null;
      this.src = "slow_rider_product_images/placeholder.png";
    };
   ...
   * THUMBNAILS
   */
  const thumbButtons = root.querySelectorAll("[data-gallery-src]");

  thumbButtons.forEach((btn, i) => {

    const imagePath = btn.dataset.gallerySrc;
