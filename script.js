...
function activateFilter(filter) {
  document.querySelectorAll('.side-item, .sub-items button').forEach(function (button) {
    button.classList.toggle('active', button.dataset.filter === filter);
  });

  const select = document.getElementById('mobileFilter');
  if (select) {
    const validFilters = [
      'all',
      'ATV',
    ...
    select.value = validFilters.includes(filter) ? filter : 'all';
  }
...
(function(){
  function removeSearchInfo(){
    document.querySelectorAll('.search-results-info').forEach(function(el){ el.remove(); });
  }
  ...
  function showSearchInfo(count, term){
    ...
    info.textContent=(count===1?'1 product found for "':'') + (count===1?term+'"':count+' products found for "'+term+'"');
  ...
  function searchProducts(term){
    ...
    const root=document.getElementById('catalogView');
    if(!root) return;
    if(!search){ renderCatalog('all'); removeSearchInfo(); return; }

    const results=products.filter(function(p){
      const specs=Object.entries(p.specs||{}).map(function(pair){return pair[0]+' '+pair[1];}).join(' ');
      const text=[p.name,p.category,p.type,p.description,specs].join(' ').toLowerCase();
      return text.includes(search);
    });

    root.innerHTML=sectionBlock('Search Results','🔍',results,'orange');
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  featured.querySelectorAll('.catalog-product-image').forEach(function(img,i){
    if(featuredProducts[i]) loadProductImage(img,featuredProducts[i]);
  });
});
...
document.addEventListener('DOMContentLoaded',function(){
  ...
  if(!featuredATVs) return;
  const atvProducts=products.filter(function(p){return p.category==='ATV';}).slice(0,4);
  featuredATVs.innerHTML=atvProducts.map(productCard).join('');
  featuredATVs.querySelectorAll('.catalog-product-image').forEach(function(img,i){
    if(atvProducts[i]) loadProductImage(img,atvProducts[i]);
  });
});
